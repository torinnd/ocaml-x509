...
## DRAFT: replacement DN attribute value API

This is a breaking alternative for discussion, not a compatibility layer. It
builds on the issuer-name fix from PR #184. Every
...
**Validation:** added synthetic DER tests for all six tags (including actual
...
issuer-name hash expectation. Existing consumers and the intermediate-issuer
regression are migrated. Static call-site checks, `git diff --check`, independent
DER length/tag/content checks with OpenSSL ASN.1 parsing, and an independent SHA1
calculation passed. OCaml 5.2 parsing checks also pass. Full compilation and test
execution are pending: the standalone test dependencies were unavailable. Run
the upstream test suite before relying on this draft.
