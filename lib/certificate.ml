...
module Asn = struct
  ...
  let tBSCertificate =
    ...
    @ (required ~label:"issuer"        @@ Distinguished_name.Encoded.Asn.name)
...
let pp' pp_custom_extensions ppf { asn ; _ } =
  ...
  Fmt.pf ppf "X.509 certificate@.version %a@.serial %a@.algorithm %a@.issuer %a@.valid from %a until %a@.subject %a@.extensions %a"
    ...
    Distinguished_name.pp (Distinguished_name.Encoded.to_legacy_lossy tbs.issuer)
    (Ptime.pp_human ~tz_offset_s:0 ()) (fst tbs.validity)
    (Ptime.pp_human ~tz_offset_s:0 ()) (snd tbs.validity)
    Distinguished_name.pp (Distinguished_name.Encoded.to_legacy_lossy tbs.subject)
...
let issuer_encoded { asn ; _ } = asn.tbs_cert.issuer

let subject_encoded { asn ; _ } = asn.tbs_cert.subject

let issuer cert = Distinguished_name.Encoded.to_legacy_lossy (issuer_encoded cert)

let subject cert = Distinguished_name.Encoded.to_legacy_lossy (subject_encoded cert)
...
let hostnames { asn = cert ; _ } =
  let subj =
    match Distinguished_name.common_name
        (Distinguished_name.Encoded.to_legacy_lossy cert.tbs_cert.subject) with
    | None -> Host.Set.empty
    | Some x ->
